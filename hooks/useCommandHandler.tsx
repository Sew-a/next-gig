...
import { TERMINAL_RESPONSES, TerminalCommand } from '@/utils/terminalCom';
...
export function useCommandHandler({ history, push, clear }: UseCommandHandlerProps) {
  ...
  const execute = (raw: string) => {
    ...
    switch (mainCmd as TerminalCommand) {
      ...
      case TerminalCommand.ROUTE: {
        ...
        } else if (!VALID_ROUTES.includes(page as typeof VALID_ROUTES[number])) {
          push(`Error: Page "${page}" not found. Try one of: ${VALID_ROUTES.join(', ')}`);
        } else {
